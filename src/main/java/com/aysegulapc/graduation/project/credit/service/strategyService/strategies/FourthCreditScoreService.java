...
@Service
@Slf4j
public class FourthCreditScoreService implements CreditStrategy {
    @Autowired
    ...
    public void calculateCreditResult(User user) {
        ...
        int res = salary.compareTo(new BigDecimal(10000));

        if(creditScore >= 500L && creditScore <= 1000L && res > 0) {
            log.info("User's information is in the fourth strategy service.");
            creditService.createCreditResponseDtoByStrategy(
                     user, user.getId(), creditScore, CreditEnum.ONAY, user.getSalary().multiply(new BigDecimal(2)), 25L);
    ...
    public StrategyNames getStrategy() {
        return StrategyNames.FourthCreditScoreService;
    }
}
